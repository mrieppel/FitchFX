...
The function Line(..) immediately below shows the attributes Line objects are expected to have.  Below that are global variables that hold the proof, conclusion aimed for, and sub-goal lines the user enters.  A proof is array of Line objects.

Internally, the program uses the "plain" notation for connectives, e.g. '>' and '<>', rather than the unicode variants.  Unicode variants are used (i) when writing proof lines to the drv sig, (ii) when exporting the proof (if the user has selected "pretty print"), and (iii) for parts of error messages written to the error console.  Proofs can be imported in plain notation.  The transformations between unicode and plain notation are done via the functions richardify() and gRul().  The parser can parse formulas in either notation, but again, the "internals" of the program expect to encounter formulas in the plain notation.

Internally, the program also uses the terminology of "SI/TI" rules ("Sequent/Theorem Introduction", from Forbes) for what in the user interface are now called "Derived Rules."
*/
	...
	if(plain || pretty) {
		...
		for(var i=0;i<PROOF.length;i++) {
			if(olin[i].length==0) {
				...
				proof += ocnt[i]+w(mc-ocnt[i].length)+ofrm[i]+w(mf-ofrm[i].length)+orul[i]+'  '+olin[i]+'\r\n';
			}
		}
	}
	if(latex) {
...
function extract_proof() {
	...
	while(tmp[0].length!=0) { // collects proof lines till we run out
		...
		proof = tmp[1];
		tmp = next_line(proof);
		if(line.length==5) { // line has all five rule columns: 0cnt 1dth 2frm 3rul 4lin 
			...
			if(f==1) { // line has depth of 0: 0cnt 1frm 2rul 3lin 
...
function checkifdone() {
	...
	var lastln = PROOF[PROOF.length-1];
	if(lastln.frm==CONCLUSION[0] && (lastln.sig.length==1 && lastln.sig[0]==0)) {
